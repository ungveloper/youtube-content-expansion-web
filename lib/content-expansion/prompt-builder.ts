...
- 방향 충돌이 실제 최종 후보를 바꿀 수 있다면 Interview Gate에서 **Primary Goal / Secondary Goal / Guardrail** 순서를 사용자와 맞춘다.
- 우선순위가 정해지기 전에는 목적별로 후보를 과도하게 늘리지 말고, 공통 근거가 강한 후보만 임시 HOLD로 유지한다.
- 예를 들어 '종합 후속 콘텐츠 + 시청자 질문'은 함께 볼 수 있지만, 검색 수요와 시리즈성이 충돌한다면 어느 쪽이 이번 제작의 1차 목적인지 인터뷰로 확인한다.
